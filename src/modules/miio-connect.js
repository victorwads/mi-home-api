...
const DeviceList = [
    ...
    { ip: '10.0.0.12', type: Type.Light, name: 'victor' },
    ...
    await Promise.all(DeviceList
        ...
        .map(async device => {
            ...
            await miio.device({ address: device.ip })
                .then(connection => device.connection = connection, console.log('Connected with ', device.name, device.ip))
                .catch(err => logErr(err, device))
            return device
        })
    )
